...
class EntityOperations implements ContainerInjectionInterface {
  ...
  /**
   * Acts on a translation being created.
   *
   * Enforces the 'translation_default_moderation_state_behavior' setting on
   * translations of moderated content.
   *
   * @param \Drupal\Core\Entity\EntityInterface $translation
   *   The entity translation being created.
   *
   * @see hook_entity_translation_create()
   */
  public function entityTranslationCreate(EntityInterface $translation): void {
    if (!$this->moderationInfo->isModeratedEntity($translation)) {
      return;
    }
    /** @var \Drupal\workflows\WorkflowTypeInterface $workflow_type */
    $workflow_type = $this->moderationInfo->getWorkflowForEntity($translation)->getTypePlugin();
    $configuration = $workflow_type->getConfiguration();
    if (!empty($configuration['translation_default_moderation_state_behavior']) && $configuration['translation_default_moderation_state_behavior'] === 'default') {
      $translation->moderation_state->value = $workflow_type->getInitialState($translation)->id();
    }
  }
